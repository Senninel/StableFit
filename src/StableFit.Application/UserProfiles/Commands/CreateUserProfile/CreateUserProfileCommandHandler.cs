...
public sealed class CreateUserProfileCommandHandler : IRequestHandler<CreateUserProfileCommand, UserProfileDto>
{
    ...
    public async Task<UserProfileDto> Handle(CreateUserProfileCommand request, CancellationToken cancellationToken)
    {
        ...
        var profile = UserProfile.Create(request.UserId, request.Username, request.Name, request.Email);
    ...
    private static UserProfileDto MapToDto(UserProfile profile) =>
        new(profile.Id, profile.UserId, profile.Username, profile.Name, profile.Email,
